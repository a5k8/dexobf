## dex混淆
dex类名混淆工具（Python）
[English](https://github.com/a5k8/dexobf/blob/663eb64f5196153849304a95defceb33bcb37cdb/README_en.md)
## 说明
把dex中的所有类都放到指定包名下以混淆，同时进行类名混淆，运行产生的文件统一存放至 `/dexobf` 目录。
## 环境部署
...
```
混淆完成后，需同步替换 `AndroidManifest.xml` 中的类名，新旧类名对应关系可在 `dexobf/mapping.txt` 中查看检索。
## 目录结构
```
dexobf/
├── dexobf.py
